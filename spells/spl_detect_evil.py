...
def spl_detect_evil(sn, level, ch, victim, target):
    if victim.is_affected(merc.AFF_DETECT_EVIL):
        return

    aff = handler_game.AffectData(type=sn, duration=level, location=merc.APPLY_NONE, bitvector=merc.AFF_DETECT_EVIL)
