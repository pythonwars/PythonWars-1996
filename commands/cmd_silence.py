...
def cmd_silence(ch, argument):
    ...
    victim.sentances.tog_bit(merc.SENT_SILENCE)
    if victim.sentances.is_set(merc.SENT_SILENCE):
        victim.send("You can't use channels!\n")
        ch.send("SILENCE set.\n")
    else:
        victim.send("You can use channels again.\n")
        ch.send("SILENCE removed.\n")
